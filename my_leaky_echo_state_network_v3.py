...
def distributed(y_target, mean):
    return np.sum((y_target - mean)**2) / len(y_target)

def mean_squared_error(y_target, y_predict):
    return np.sum((y_target - y_predict)**2) / len(y_target)
...
class Leaky_echo_state_network:
    ...
    def accuracy(self):                                                                   # 精度評価
        # 学習誤差
        true_training_data = self.training_data
        true_training_data_ave = average(true_training_data)
        true_training_data_distributed = distributed(true_training_data, true_training_data_ave)
        trained_data = np.delete(self.trained_data, 0)
        MSE_trained = mean_squared_error(true_training_data, trained_data)
        NRMSE_trained = np.sqrt(MSE_trained / true_training_data_distributed)
        print("Training error =",NRMSE_trained)
        # 予測誤差
        true_test_data = self.test_data
        true_test_data_ave = average(true_test_data)
        true_test_data_distributed = distributed(true_test_data, true_test_data_ave)
        predict_data = self.output_data
        MSE_predict = mean_squared_error(true_test_data, predict_data)
        NRMSE_predict = np.sqrt(MSE_predict / true_test_data_distributed)
        print("Prediction error =", NRMSE_predict)

    def change_step(self, step_num):
        predict_data_changed = self.output_data[step_num:]
        test_time_data_changed = self.test_time_data[:-step_num]

        true_data = self.input_data[self.training_data_size:-step_num]
        true_data_ave = average(true_data)
        true_data_distributed = distributed(true_data, true_data_ave)
        MSE_predict_changed = mean_squared_error(true_data, predict_data_changed)
        NRMSE_predict_changed = np.sqrt(MSE_predict_changed / true_data_distributed)
        print(f"{step_num} time steps NRMSE =", NRMSE_predict_changed)

        def __graph(time, data, name, color):                                       
            plt.plot(time, data, linewidth=2, label=name, color=color)

        fig = plt.figure(figsize=(6, 4))                                                 # plt.figure()のfigsizeは(width, height)のタプルを渡し、単位はインチ
        ax = fig.add_subplot(111)                                                         # グラフの描写領域設定 (111)は１行目１列１番目という意味で、subplot(1,1,1)も同じ
        ax.set_title('Time step gap', fontsize=15)                                        # サブプロットaxのタイトル設定
        ax.set_xlabel('Time step', fontsize=12)                                           # axのx軸タイトル設定
        ax.set_ylabel('Amplitude', fontsize=12)                                           # axのy軸タイトル設定
        plt.tick_params(labelsize=8)

        __graph(self.time_data[self.training_data_size:-step_num], self.input_data[self.training_data_size:-step_num], "input", 'blue')

        if predict_data_changed is not None:
            __graph(test_time_data_changed, predict_data_changed, f"Step_num = {step_num}", 'orange')
...
if __name__ == '__main__':                                                                # このファイルがコマンドラインからスクリプトとして実行された場合にのみ以降の処理を実行する
    ...
    network.predict()
    network.accuracy()
    network.change_step(5)
    network.change_step(10)
    network.change_step(20)
    network.change_step(30)
    network.change_step(40)
    network.accuracy()
    network.graph()
